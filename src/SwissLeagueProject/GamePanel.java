...
public class GamePanel extends JPanel implements ActionListener, ItemListener {
	...
	public GamePanel(SwissLeagueFrame swissLeagueFrame) {
	...
	public void start_game(/*String[] people, int[] record_rd, double[][] rank, int[][] record_fight, double[][] total, int round_now*/) {
		...
		for(int i = 0; i < people_num; i++) {
			...
			// 여기서 승패를 가리고 순위를 나눠야 하는 rank배열에 승점,승리횟수,버치값,버그값 입력
				...
				System.out.println(people[second[check]]);
	...
	public void buch(double[][] total, int[][] record_fight, int round_now, int first, int second, double[][] rank) {
		...
		for(int i = 0; i < round_now + 1; i++) {
			...
			System.out.println(record_fight[first][i] + " << 왼쪽 선수와 대진했던 선수번호 이름 >> " + people[(int)total[record_fight[first][i]][0]]);
			System.out.println(sum + " 점수");
		}
		for(int i = 0; i < people_num; i++) {
		...
		for(int i = 0; i < round_now + 1; i++) {
			...
			System.out.println(record_fight[second][i] + " << 오른쪽 선수와 대진했던 선수번호 이름 >> " + people[(int)total[record_fight[second][i]][0]]);
			System.out.println(sum + " 점수");
		}
		for(int i = 0; i < people_num; i++) {
	...
	public void itemStateChanged(ItemEvent e) {
		...
		for(int i = 0; i < people_num / 2; i++) {
			...
			if(left_win[i].isSelected()) {
				...
				System.out.println("왼쪽 선택");
				System.out.println(people[first[check]] + " 왼쪽 선수");
				System.out.println(people[second[check]] + " 오른쪽 선수");
	...
	public void League_Table() {
		...
		for(int i = 0; i < people_num; i++) {
			...
			if(record_fight[first[check]][round_now] == -1) {
				//  왼쪽 선수 출력
